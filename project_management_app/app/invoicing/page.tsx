...
export default function InvoicingPage() {
  ...
  const handleConfirmInvoiced = () => {
    console.log(
      "Confirming invoiced for projects:",
  ...
  const filteredProjects = sortedProjects.filter((project) => {
    ...
    if (activeTab === "toBeInvoiced") {
      if (project.invoiced) return false;
    } else if (activeTab === "toBePaid") {
      if (!project.invoiced || project.paid) return false;
    }
    ...
    if (
      sourceLanguageFilter &&
      project.sourceLanguage !== sourceLanguageFilter
    ) {
      return false;
    }
    ...
    if (
      targetLanguageFilter &&
      project.targetLanguage !== targetLanguageFilter
    ) {
      return false;
    }
    ...
    if (
      lengthFilter &&
      project.words &&
      !matchesLengthFilter(project.words, lengthFilter)
    ) {
      return false;
    }
  ...
  const tabs = [
    ...
    {
      id: "toBeInvoiced" as TabType,
      label: "To be Invoiced",
      count: projects.filter((p) => !p.invoiced).length,
    },
    {
      id: "toBePaid" as TabType,
      label: "To Be Paid",
      count: projects.filter((p) => p.invoiced && !p.paid).length,
    },
  ];
            ...
            <div className="flex gap-3">
              ...
              {(activeTab === "all" || activeTab === "toBePaid") && (
                <button
                  ...
                  className="px-6 py-3 cursor-pointer bg-green-500 hover:bg-green-600 text-white rounded-lg transition-colors shadow-sm"
                  type="button"
                >
                  Confirm Paid
                </button>
